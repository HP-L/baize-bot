...
void ST7789_Init(void) {
    ...
    ST7789_WriteSmallData(0x32);			//	1.35V (default 0.75v for 0x20)
    ST7789_WriteCommand(0xC0);				//	LCMCTRL
    ST7789_WriteSmallData(0x2C);			//	Default value
    ST7789_WriteCommand(0xC2);				//	VDV and VRH command Enable
    ST7789_WriteSmallData(0x01);			//	Default value
    ST7789_WriteCommand(0xC3);				//	VRH set
    ST7789_WriteSmallData(0x15);			//	+-4.8v (defalut +-4.1v for 0x0B)
 ...
 * @param font -> 数字的字体样式。
 * @param color -> 数字的颜色。
 * @param bgcolor -> 数字的背景颜色。
 * @return none
 */
void ST7789_DisplayNumber(uint16_t x, uint16_t y, int number, FontDef font, uint16_t color, uint16_t bgcolor) {
	    char buffer[16];
	    snprintf(buffer, sizeof(buffer), "%d", number);
	    int len = strlen(buffer);
	    int xPos = 0;
	    /* 先清除之前可能残留的内容 */
//	    ST7789_DrawFilledRectangle(x + len * font.width, y, x + (len+1) * font.width, font.height, bgcolor);
	    for (int i = 0; i < len; i++) {
	        ST7789_WriteChar(x + xPos, y, buffer[i], font, color, bgcolor);
	        xPos += font.width; // 根据字体宽度移动下一个字符的位置
	        ST7789_WriteString(x + xPos, y, " ", font, color, bgcolor);
	    }
//        ST7789_WriteChar(x + xPos + font.width, y, " ", font, color, bgcolor);
}
